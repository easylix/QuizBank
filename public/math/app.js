...
async function apiQuery(params = {}) {
  ...
  const url = API_BASE + '/query' + (searchParams.toString() ? '?' + searchParams.toString() : '');
...
async function loadQuestionsFromCloud() {
  try {
    const res = await apiQuery({limit: 1000});
    if (res.success && res.data) {
      // Also load detail for each question to get images when viewing list (lazy)
      const seen = new Set();
...
async function saveNumberToTag(id, number) {
  // Number is stored directly in the tags field, handled in saveUpdateQuestion
}

async function removeNumberFromTag(id) {
  // Number removal is handled in saveUpdateQuestion
}

async function saveAddQuestion(data) {
  try {
    const tags = data.tags || '';
    const fullTags = data.number ? (tags ? tags + ',number:' + data.number : 'number:' + data.number) : tags;
...
function renderMath(text) {
  ...
  while ((m = combined.exec(text)) !== null) {
    ...
    if (m[1]) {
      ...
      // 块级公式 \[...\]
      try {
        result += window.katex.renderToString(m[4].trim(), { displayMode: true, throwOnError: false });
      } catch(e) {
        result += m[3];
      }
    } else if (m[5]) {
      // 行内公式 $...$
      try {
        result += window.katex.renderToString(m[5].trim(), { displayMode: false, throwOnError: false });
      } catch(e) {
        result += m[0];
      }
    } else if (m[6]) {
      // 行内公式 \(...\)
      try {
        result += window.katex.renderToString(m[6].trim(), { displayMode: false, throwOnError: false });
      } catch(e) {
        result += m[0];
      }
    }
...
function replaceFigMarkers(text, q) {
  ...
  function getFigSrc(kind, index) {
    ...
    if (fieldVal && fieldVal.trim()) {
      // data URI (base64): whole field is one image, don't split
      if (fieldVal.trim().startsWith('data:')) {
        return resolveSrc(fieldVal.trim());
      }
